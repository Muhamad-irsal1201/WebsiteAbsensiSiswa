<?php
// php/add_lesson.php
// Tambah jadwal pelajaran: inisialisasi variabel, cek bentrok, saran slot kosong (AJAX)
...
// inisialisasi pesan untuk menghindari "Undefined variable"
$error = "";
$success = "";
...
if (isset($_GET['action']) && $_GET['action'] === 'suggest' && isset($_GET['day'])) {
    ...
    $stmt = mysqli_prepare($db, "SELECT time_start, time_end, teacher, class FROM lesson_schedule WHERE day = ?");
...
/**
 * Proses submit form untuk insert jadwal
 */
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($class === '' || $subject === '' || $teacher === '' || $day === '' || $time_start === '' || $time_end === '') {
        ...
        if (!preg_match('/^\d{2}:\d{2}$/', $time_start) || !preg_match('/^\d{2}:\d{2}$/', $time_end)) {
            ...
            mysqli_stmt_bind_param($stmt2, "sssss", $day, $teacher, $class, $time_start, $time_end);
            ...
            if (mysqli_num_rows($resCheck) > 0) {
                ...
                $sqlInsert = "INSERT INTO lesson_schedule (class, subject, teacher, day, time_start, time_end) VALUES (?,?,?,?,?,?)";
                $stmt3 = mysqli_prepare($db, $sqlInsert);
                if ($stmt3) {
                    mysqli_stmt_bind_param($stmt3, "ssssss", $class, $subject, $teacher, $day, $time_start, $time_end);
                    $ok = mysqli_stmt_execute($stmt3);
                    if ($ok) {
                        mysqli_stmt_close($stmt3);
                        header("Location: lesson.php?class=" . urlencode($class) . "&added=1");
                        exit;
                    } else {
  ...
  <form method="post" action="">
    <div class="form-group">
      ...
      <select id="class" name="class" required>
        ...
        <option value="VII" <?php if(isset($_POST['class'])&&$_POST['class']=='VII') echo 'selected'; ?>>VII</option>
        <option value="VIII" <?php if(isset($_POST['class'])&&$_POST['class']=='VIII') echo 'selected'; ?>>VIII</option>
        <option value="IX" <?php if(isset($_POST['class'])&&$_POST['class']=='IX') echo 'selected'; ?>>IX</option>
      </select>
    </div>
    ...
    <div class="form-group">
      ...
      <select id="subject" name="subject">
        ...
        <?php foreach($teachers as $t): ?>
          <option value="<?php echo htmlspecialchars($t['subject']); ?>" <?php if(isset($_POST['subject']) && $_POST['subject']==$t['subject']) echo 'selected'; ?>>
            <?php echo htmlspecialchars($t['subject']); ?>
          </option>
        <?php endforeach; ?>
      </select>
      <input type="text" name="subject_new" placeholder="Atau ketik mapel baru" value="<?php echo isset($_POST['subject_new'])?htmlspecialchars($_POST['subject_new']):''; ?>" />
    ...
    <div class="form-group">
      ...
      <select id="teacher" name="teacher">
        ...
        <?php foreach($teachers as $t): ?>
          <option value="<?php echo htmlspecialchars($t['name']); ?>" <?php if(isset($_POST['teacher']) && $_POST['teacher']==$t['name']) echo 'selected'; ?>>
            <?php echo htmlspecialchars($t['name']); ?> (<?php echo htmlspecialchars($t['subject']); ?>)
          </option>
        <?php endforeach; ?>
      </select>
      <input type="text" name="teacher_new" placeholder="Atau ketik nama guru baru" value="<?php echo isset($_POST['teacher_new'])?htmlspecialchars($_POST['teacher_new']):''; ?>" />
      <div class="note">leave blank if selecting above</div>
    </div>
    ...
    <div class="form-group">
      ...
      <select id="day" name="day" required>
        ...
        <?php
          ...
          foreach ($days as $d) {
            $sel = (isset($_POST['day']) && $_POST['day']==$d) ? 'selected':''; 
    ...
    <div class="form-group">
      ...
      <input id="time_start" name="time_start" type="time" required value="<?php echo isset($_POST['time_start'])?htmlspecialchars($_POST['time_start']):''; ?>" />
    </div>

    <div class="form-group">
      <label for="time_end">End Time</label>
      <input id="time_end" name="time_end" type="time" required value="<?php echo isset($_POST['time_end'])?htmlspecialchars($_POST['time_end']):''; ?>" />
    </div>
...
function fetchSuggestions() {
  ...
  const url = `add_lesson.php?action=suggest&day=${encodeURIComponent(day)}&class=${encodeURIComponent(cls)}&teacher=${encodeURIComponent(teacher)}`;
  fetch(url)
    .then(r => r.json())
    .then(data => {
      suggestionsEl.innerHTML = '<option value="">-- Pilih Saran Jadwal --</option>';
